...
console.log("CDN", "Booting up CDN...");
...
cdn.use(express.static("./content/images"));
cdn.listen(port, ip);
console.log("CDN", "API started on " + ip + ":" + port);
...
cdn.use("/images/:data_type/:id", (req, res, next) => {
  ...
  if (ext)
    req.params.id = req.params.id.slice(0, req.params.id.length - ext.length);

  req.params.data_type =
    data_type_map[req.params.data_type] || req.params.data_type;
...
cdn.use("/images/:data_type", (req, res, next) => {
  ...
  if (ext)
    req.params.data_type = req.params.data_type.slice(
      0,
      req.params.data_type.length - ext.length
    );
